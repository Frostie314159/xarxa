...
fn main() {
    ...
    // `repr(align(N))` needs a literal, so the alignment goes out as a cfg the
    // buffer type matches on, not just as a constant.
    println!("cargo:rustc-check-cfg=cfg(packet_buf_align, values(\"1\", \"2\", \"4\", \"8\", \"16\", \"32\"))");
    let align = configs["PACKET_BUF_ALIGN"].value;
    if !matches!(align, 1 | 2 | 4 | 8 | 16 | 32) {
        panic!("PACKET_BUF_ALIGN must be 1, 2, 4, 8, 16 or 32, got {align}");
    }
    println!("cargo:rustc-cfg=packet_buf_align=\"{align}\"");
