// The TCP socket only uses part of this interface. The unused parts are exercised
// by the tests.
#![allow(dead_code)]
    ...
    pub fn add(&mut self, mut offset: usize, size: usize) -> Result<(), TooManyHolesError> {
        ...
        if shift != 0 {
            for x in i + 1..self.contigs.len() {
                ...
                self.contigs[x] = self.contigs.get(x + shift).copied().unwrap_or_else(Contig::empty);
            }
        }
