...
/// A compressed IPv6 address, before it is resolved with the link-layer
/// addresses and the address contexts.
#[derive(Debug, PartialEq, Eq, Clone, Copy)]
#[cfg_attr(feature = "defmt", derive(defmt::Format))]
pub enum UnresolvedAddress<'a> {
    /// Compressed against the link-local prefix.
    WithoutContext(AddressMode<'a>),
    /// Compressed against the address context with the given index.
    WithContext((usize, AddressMode<'a>)),
    /// A reserved encoding.
    Reserved,
}

/// How much of an IPv6 address is carried inline.
#[derive(Debug, PartialEq, Eq, Clone, Copy)]
#[cfg_attr(feature = "defmt", derive(defmt::Format))]
pub enum AddressMode<'a> {
    ...
    /// carried in-line.
    InLine64bits(&'a [u8]),
    /// The first 112 bits are elided. The value of the first
    ...
    /// computed from the link-layer address.
    FullyElided,
    ...
    Unspecified,
    /// An encoding this crate does not support.
    NotSupported,
}
...
impl<'a> UnresolvedAddress<'a> {
    /// Resolve the address.
    ///
    /// `ll_address` is the link-layer address the elided bits are derived
    /// from. `addr_context` holds the address contexts, indexed by context
    /// identifier.
    ///
    /// Errors:
    /// - `Error` if the encoding is reserved or unsupported, or refers to a
    ///   context or link-layer address that is not there.
    pub fn resolve(self, ll_address: Option<LlAddress>, addr_context: &[AddressContext]) -> Result<ipv6::Address> {
        let mut bytes = [0; 16];
        ...
        match self {
            UnresolvedAddress::WithoutContext(mode) => match mode {
                AddressMode::FullInline(addr) => Ok(ipv6::Address::from_octets(addr.try_into().unwrap())),
...
mod test {
    ...
    fn sixlowpan_three_fragments() {
        ...
        let ieee802154_frame = Ieee802154Frame::new_checked(&mut frame1[..]).unwrap();
        let ieee802154_repr = Ieee802154Repr::parse(&ieee802154_frame).unwrap();

        let sixlowpan_frame = SixlowpanPacket::dispatch(ieee802154_frame.payload().unwrap()).unwrap();
        assert_eq!(sixlowpan_frame, SixlowpanPacket::FragmentHeader);

        let header_len = ieee802154_frame.header_len();
        let frag = frag::Packet::new_checked(&mut frame1[header_len..]).unwrap();
        ...
        let ieee802154_frame = Ieee802154Frame::new_checked(&mut frame2[..]).unwrap();
        let ieee802154_repr = Ieee802154Repr::parse(&ieee802154_frame).unwrap();
        ...
        let header_len = ieee802154_frame.header_len();
        let frag = frag::Packet::new_checked(&mut frame2[header_len..]).unwrap();
        ...
        let ieee802154_frame = Ieee802154Frame::new_checked(&mut frame3[..]).unwrap();
        let ieee802154_repr = Ieee802154Repr::parse(&ieee802154_frame).unwrap();
        ...
        let header_len = ieee802154_frame.header_len();
        let frag = frag::Packet::new_checked(&mut frame3[header_len..]).unwrap();
